...
class MiniClientsController < ApplicationController
  ...
  def ajax_index_table_v4
    #pp params
    #pp "99999999999999999999"
    search_hash = {
        :client_name_or_client_number_like => params[:search_val],
        :serve_user1_id_eq=>params[:search_serve_user1_id],
        :user_id_eq => params[:search_user_id]
    }
    @search = MiniClient.includes(:client,:serve_records).search(search_hash)
    set_ajax_index_data_v4
  ...
  def ajax_lose
    ...
    if request.post? && @mini_client.update_attribute(:is_losed,true)
      ...
      #end
      render :partial => "/mini_clients/ajax_index/ajax_index_table_v4_tr",:locals => {:client=>@mini_client}
  ...
  def distribute_client_to_server
    if request.post?
      ...
      #pp @mini_client.serve_user1_name
  ...
  def ajax_show
    ...
    @contact_records = @mini_client.contact_records.where(condition_string).first(10)
    @serve_records = @mini_client.serve_records.includes(:order).first(10)
  ...
  def set_ajax_index_data_v4
    ...
    if @current_user.had_permission?(30)
      where_conditions = "mini_clients.status_num = 4"
      if params[:cate] == "1"
        where_conditions << " and is_losed = 1"
      else
        where_conditions << " and is_losed = 0"
      end

      #pp "88888888888888"
      #pp where_conditions

      @mini_clients_count = @search.where(where_conditions).count
      @mini_clients = @search.where(where_conditions).order(params[:sort]).page(params[:page]).per(@every_page_count)
    else
      users = User.subordinate_users(@current_user)
      user_ids = users.map{|u|u.id}.join(",")
      where_conditions = "(mini_clients.serve_user1_id in (#{user_ids}) or mini_clients.serve_user2_id in (#{user_ids})) and mini_clients.status_num = 4"
      if params[:cate] == "1"
        where_conditions << " and is_losed = 1"
      else
        where_conditions << " and is_losed = 0"
